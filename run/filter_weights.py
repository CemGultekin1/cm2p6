...
def run():
    datargs = sys.argv[1:]
    generator = get_filter_weights_generator(datargs,data_loaders = True)
    filename = get_filter_weights_location(datargs,preliminary=True)
    print(f'filename = {filename}')
    args,_ = options(datargs,key = "run")
    time = Timer()
    time.start('data')
    fweights = None
    for t,(coords,locs,fw) in enumerate(generator):
        time.end('data')
