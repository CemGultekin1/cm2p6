...
def get_lsrp_values(stats):
    return drop_unused_coords(stats.isel(seed = 0,latitude = 0, domain = 0, temperature = 0,lsrp = 0,training_depth = 0,co2 = 0,model = 1))

def depth_plot(stats):
    lsrp_ = get_lsrp_values(stats)#.isel(sigma= range(1,4))
    ...
    stats_ = stats.isel(seed = 0,latitude = 0, \
            domain = 1, temperature = 1, sigma = range(4),co2= 0,training_depth = range(1,8),)
    ...
    for sigma_i,r2corr in itertools.product(range(4),range(2)):
        ...
        fcnn = stats.isel(model = 0,lossfun = 0)
        ...
        for i,j in itertools.product(range(nrows),range(ncols)):
            ...
            if i==0:
                title = ['FCNN','FCNN+LSRP'][j]
                title = r2corr_str.upper() + ', ' + title + f', \u03C3={sigma}'
                ax.set_title(title)
            ax.set_xlabel('depth (m)')

        fig.savefig(f'saves/plots/depth_comparison/{r2corr_str}_{sigma}.png')
        print(f'saves/plots/depth_comparison/{r2corr_str}_{sigma}.png')
